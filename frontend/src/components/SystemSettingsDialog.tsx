    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl border-cyan-300/12 bg-[linear-gradient(180deg,rgba(8,15,28,0.98),rgba(7,16,31,0.95))]">
        <DialogHeader className="pr-10">
          ...
          <DialogDescription>
            这里统一管理 LLM Provider、API Key 和主题模式。配置会写入本地存储，并同步到当前任务控制区。
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="mt-6 grid gap-5">
          ...
          <section className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/55">Theme Mode</p>
                <p className="mt-2 text-sm text-white/65">使用 `next-themes` 在暗色和亮色模式之间切换。</p>
              </div>
